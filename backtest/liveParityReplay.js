...
async function run() {
    ...
    for (const tsMs of timeline) {
        ...
        for (const symbol of activeUniverse) {
            ...
            const riskAmount = toNum(plan.riskAmount) ?? equity * nextRiskPct;
            if (![entryPrice, sl, tp, size, riskAmount].every(Number.isFinite)) continue;
    ...
    for (const w of weekRows) {
        weeklyPairPivotMap.set(isoDate(w.weekStartMs), {
            week: isoDate(w.weekStartMs),
            EURUSD: 0,
            GBPUSD: 0,
            USDJPY: 0,
            USDCHF: 0,
    ...
    const weeklyPairTable = renderTable(
        ["Week", "EURUSD", "GBPUSD", "USDJPY", "USDCHF", "Total"],
